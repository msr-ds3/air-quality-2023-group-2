...
df_no2_pollution <- df[df$parameter == "no2", ]
df_o3_pollution <- df[df$parameter == "o3", ]
df_pm25_pollution <- df[df$parameter == "pm25", ]
...
ggplot(data = df_pm25, aes(y = countryCode, x = mean_change, color = mean_change)) +
  ...
  geom_vline(xintercept = 0, linetype = "dashed", color = "black") +
  geom_errorbarh(aes(xmin = mean_change - se, xmax = mean_change + se), height = 0.2) +
  scale_color_gradient2(low = "purple4", high = "orange") +
